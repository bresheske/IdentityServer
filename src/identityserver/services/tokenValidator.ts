...
export class TokenValidator {
    ...
    public async isValid(token:string): Promise<boolean> {
        ...
        try {
            ...
            let dec = publickey.decryptPublic(authtoken.signature, 'utf8');

            // Now we hash the token and compare it against the signature.
            let tokenstring = JSON.stringify(authtoken.identity);
            ...
            let now = new Date();
            let expires = new Date(authtoken.identity.expires);
            if (now >= expires)
                return false;
